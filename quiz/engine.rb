  ...
  class Engine
    ...
    def stop(message)
      ...
      if percent >= 0 && percent < 20
        @bot.api.send_message(chat_id: message.chat.id, text: "Треба повчитися. Рекомендується ознайомитися з основними принципами і практиками інформаційної гігієни і докласти зусиль для поліпшення своїх знань та навичок.", reply_markup: kb)
      elsif percent >= 20 && percent < 40
        @bot.api.send_message(chat_id: message.chat.id, text: "Не погано, але можете краще. Ви маєте базові знання про інформаційну гігієну, але ще є простір для вдосконалення та поглиблення своїх знань.", reply_markup: kb)
      elsif percent >= 40 && percent < 60
        @bot.api.send_message(chat_id: message.chat.id, text: "Молодець. Ви володієте прийнятним рівнем знань про інформаційну гігієну і вмієте застосовувати деякі практики в своєму щоденному житті. Продовжуйте розвиватися в цій області.", reply_markup: kb)
      elsif percent >= 60 && percent < 80
        @bot.api.send_message(chat_id: message.chat.id, text: "Чемпіон. Ви добре ознайомлені з інформаційною гігієною і успішно застосовуєте її принципи в практичних ситуаціях. Ваші знання є глибшими і ширшими.", reply_markup: kb)
      elsif percent >= 80
        @bot.api.send_message(chat_id: message.chat.id, text: "Просто найкращий гуру. Ви маєте високий рівень знань та експертизи в галузі інформаційної гігієни. Ви володієте всіма необхідними навичками та розумінням для ефективного застосування інформаційної гігієни у своєму житті та роботі. Ви справжній фахівець!", reply_markup: kb)
      end
